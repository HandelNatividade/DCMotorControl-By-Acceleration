...
const int controlInterval = 100;  // 10Hz
...
double filteredPwr = 0.0;   // EMA da potência
double alpha = 0.03;        // fator do filtro (entre 0.05 e 0.3)
...
void loop() {
    ...
    switch (currentState) {
        case STATE_IDLE:
            runSystemLoop();
            break;
        case STATE_CALIB_OFFSET:
            runCalibOffsetLoop();
            break;
        case STATE_CALIB_STEP:
            runCalibStepLoop();
            break;
        case STATE_PRBS:
            runPRBSLoop();
...
void runPRBSLoop() {
    ...
    if (now - lastSampleTime >= 5) {
        ...
        double pwr = (event.acceleration.x * event.acceleration.x) +
                     (event.acceleration.y * event.acceleration.y) +
                     (event.acceleration.z * event.acceleration.z);
        energyAccumulator += pwr;
        sampleCount++;
    }
   
    if (now - lastPrbsChange >= Ts_prbs) {
        lastPrbsChange = now;
        uint8_t bit = ((lfsr >> 6) ^ (lfsr >> 5)) & 0x01;
        lfsr = (lfsr << 1) | bit;
        if (lfsr & 0x01) {
            pwmOutput = prbs_pwm_max;
        } else {
            pwmOutput = prbs_pwm_min;
        }
...
void runSystemLoop() {
    ...
    if (now - lastControlTime >= controlInterval) {
        ...
        // -------- PID COM BACK-CALCULATION (ANTI-WINDUP REAL) --------
        if (runFlag == 1) {
            ...
            // ERRO EM POTÊNCIA (por enquanto)
            double error = setpoint - filteredPwr;


            // Termos
            double P = Kp * error;
            double D = Kd * (error - err_prev);
            err_prev = error;
        ...
        if (measureState > 0) {
            ...
            double estRPM = estimarRpmPelaTensao(vVib);
...
void parseCommand(String cmd) {
    ...
    if (cmd == "prbs") {
        ...
        measureState = 0;
        stopMotor();
        currentState = STATE_PRBS;
        lfsr = 0x54;
        lastPrbsChange = millis();
        return;
    }
    ...
    if (cmd.startsWith("medir")) {
        if (cmd.indexOf(',') > 0) {
            ...
            measureState = mode;
            if (mode == 1) {
                Serial.println("Tempo\tDuty\tNetPwr\tV_PWM\tV_Vib\tRPM");
            } else {
    ...
    if (cmd.charAt(0)=='P' && cmd.indexOf(',')>0) {
    ...
    if (cmd.charAt(0)=='I' && cmd.indexOf(',')>0) {
    ...
    if (cmd.charAt(0)=='D' && cmd.indexOf(',')>0) {
...
void startCalibrationRoutine() {
    ...
    calibSamplesCount=0;
    Serial.println("Calibrando Offset (60s)...");
}
...
void runCalibOffsetLoop() {
    if (millis() - lastCalibSampleTime >= Ts_calib) {
        ...
        sensors_event_t ev;
        accel.getEvent(&ev);
        sumX += ev.acceleration.x;
        sumY += ev.acceleration.y;
        sumZ += ev.acceleration.z;
    ...
    if (millis() - calibStartTimer >= calibDuration) {
        ...
        Serial.print("PWR_BASE (Raw): ");
        Serial.println(PWR_BASE);
        startStepMode();
    }
}
...
void runCalibStepLoop() {
    if (millis() - lastCalibSampleTime >= Ts_calib) {
        ...
        sensors_event_t ev;
        accel.getEvent(&ev);
        double pwr = ev.acceleration.x*ev.acceleration.x + ev.acceleration.y*ev.acceleration.y + ev.acceleration.z*ev.acceleration.z;
        pwr_sum += pwr;
        pwr_sq_sum += pwr*pwr;
        pwr_samples++;
    }
...
port = 'COM11'; % <--- VERIFIQUE SUA PORTA
baudrate = 115200;
try
   ...
   disp('Erro ao abrir serial. Verifique se o Arduino IDE está fechado.');
   return;
end
pause(2); % Aguarda o Arduino reiniciar
%% 2. INICIALIZAÇÃO DO MODO DE DADOS
% --- AQUI ESTÁ O COMANDO SOLICITADO ---
disp('Configurando Arduino para modo de dados...');
writeline(s, 'medir,1');
pause(0.5); % Tempo para o Arduino processar e imprimir o cabeçalho
flush(s);   % Limpa o buffer para remover o cabeçalho de texto inicial
%% 3. DEFINIÇÃO DO PERFIL DE VELOCIDADE (SETPOINT)
Ts = 0.05;       % Taxa de atualização do MATLAB (50ms)
runTime = 75;    % Duração (segundos)
time = 0:Ts:runTime;
N = numel(time);
% Onda Quadrada de RPM
% Mínimo: 525 RPM (Bem acima da deadzone)
% Máximo: 1630 RPM
levels = [850 1200  1630]; 
step_duration = 25; % segundos cada patamar
rpm_target = zeros(size(time));
for i = 1:length(time)
   idx = floor(time(i) / step_duration) + 1;
   if idx > length(levels)
       idx = length(levels);
   end
   rpm_target(i) = levels(idx);
end
%% 4. PREPARAR GRÁFICOS
f = figure('Name', 'Controle de RPM em Malha Fechada', 'Position', [100 100 900 600]);
% Gráfico 1: Velocidade
subplot(2,1,1);
hSet = animatedline('Color', 'k', 'LineWidth', 1.5, 'LineStyle', '--');
hReal = animatedline('Color', 'b', 'LineWidth', 2);
title('Resposta de Velocidade');
ylabel('RPM'); xlabel('Tempo (s)');
legend('Setpoint', 'Real (Vibração)');
grid on; ylim([0 2000]);
% Gráfico 2: Esforço de Controle (PWM)
subplot(2,1,2);
hPWM = animatedline('Color', 'r', 'LineWidth', 1.5);
title('Esforço de Controle');
ylabel('PWM (0-255)'); xlabel('Tempo (s)');
grid on; ylim([0 260]);
%% 5. LOOP DE CONTROLE
disp('Iniciando controle...');
filename = "log_rpm_" + string(datetime('now','Format','HHmmss')) + ".txt";
fid = fopen(filename, 'wt');
current_rpm_cmd = 0;
try
   for k = 1:N
      
       % tempo controlado pelo MATLAB
       t_local = (k-1)*Ts;
       % 1. Enviar Setpoint
       ref = rpm_target(k);
       if abs(ref - current_rpm_cmd) > 1
           cmd = "rpm," + num2str(round(ref));
           writeline(s, cmd);
           current_rpm_cmd = ref;
       end
       % 2. Ler resposta do Arduino
       if s.NumBytesAvailable > 0
           line = readline(s);
           vals = str2double(strsplit(line, '\t'));
           if length(vals) >= 6 && ~isnan(vals(1))
               pwm_val = vals(2);
               rpm_est = vals(6);
               % salvar tempo local (não o do Arduino)
               fprintf(fid, '%.3f %.0f %.2f %.0f\n', t_local, pwm_val, rpm_est, ref);
               % plot usando tempo local
               addpoints(hSet, t_local, ref);
               addpoints(hReal, t_local, rpm_est);
               addpoints(hPWM, t_local, pwm_val);
               drawnow limitrate;
           end
       end
       pause(0.02);
   ...
   disp('Loop interrompido.');
end
%% 6. FINALIZAR
writeline(s, 'stop'); % Para o motor
fclose(fid);
clear s;
disp('Fim.');
